...
impl ModelsManagerImpl for Arc<dyn ModelsManager> {
    ...
    fn list_collaboration_modes(
        ...
        Box::pin(async move { ModelsManager::list_collaboration_modes(self.as_ref()) })
    }
}
...
mod tests {
    ...
    use codex_protocol::{ThreadId, config_types::ModeKind, protocol::ThreadGoal};
                ...
                let has_implementation = thread.ops.lock().unwrap().iter().any(|op| {
                    matches!(
                        ...
                        Op::UserInputWithTurnContext {
                            ...
                            approval_policy: Some(_),
                            permission_profile: Some(_),
                            active_permission_profile: Some(_),
                            ..
                        } if mode.mode == ModeKind::Default
                            && matches!(
                                items.as_slice(),
            ...
            loop {
                let has_full_access_implementation = thread.ops.lock().unwrap().iter().any(|op| {
                    matches!(
                        ...
                        Op::UserInputWithTurnContext {
                            collaboration_mode: Some(mode),
                            active_permission_profile: Some(active_profile),
                            ..
                        } if mode.mode == ModeKind::Default
                            && active_profile.id == CODEX_DANGER_NO_SANDBOX_PROFILE_ID
                    )
                });
    ...
    #[tokio::test]
    async fn test_request_user_input_chains_questions_before_submitting_answers()
    -> anyhow::Result<()> {
        let session_id = SessionId::new("test");
        let client = Arc::new(StubClient::with_permission_responses(vec![
            RequestPermissionResponse::new(RequestPermissionOutcome::Selected(
